...
use zune_core::colorspace::ColorSpace::RGB565;
    ...
    pub(crate) fn post_process(
        ...
        let mut color_conv_function =
            |num_iters: usize, samples: [&[i16]; 4]| -> Result<(), DecodeErrors> {
                let tmp_out_colorspace_components = if self.options.jpeg_get_out_colorspace() == RGB565{
                    2
                }else{
                    out_colorspace_components
                };

                for (pos, output) in pixels[px..]
                    .chunks_exact_mut(width * tmp_out_colorspace_components)
                    .take(num_iters)
                    .enumerate()
                {
                    ...
                    px += width * tmp_out_colorspace_components;
                }
                Ok(())
            };
