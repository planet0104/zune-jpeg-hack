    ...
    T: ZByteReaderTrait
{
    ...
    /// # Arguments
    ///  - `stream`: The raw bytes of a jpeg file.
    pub fn new_with_output_colorspace(stream: T, color_space: ColorSpace) -> JpegDecoder<T> {
        let mut decoder = JpegDecoder::default(DecoderOptions::default(), stream);
        decoder.color_convert_16 = choose_ycbcr_to_rgb_convert_func(color_space, &DecoderOptions::default()).unwrap();
        decoder
    }
        ...
        return if self.headers_decoded {
            Some(
                usize::from(self.width())
                    ...
                    .checked_mul(if self.options.jpeg_get_out_colorspace() == ColorSpace::RGB565{
                        2
                    }else{
