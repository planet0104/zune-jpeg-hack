...
) -> Result<(), DecodeErrors> // so many parameters..
{
    ...
    match (input_colorspace, output_colorspace) {
        ...
        (
            ColorSpace::YCbCr,
            ColorSpace::RGB | ColorSpace::RGBA | ColorSpace::BGR | ColorSpace::BGRA | ColorSpace::RGB565
        ) => {
            color_convert_ycbcr(
                unprocessed,
                width,
...
fn color_convert_ycbcr(
    ...
    let num_components = if output_colorspace == ColorSpace::RGB565{ 2 }else 
        ...
        .zip(output.chunks_exact_mut(stride))
    {
        if width < 16 {
            ...
            // copy to stride
            let tmp = &temp[0..width * num_components];
            out[0..width * num_components].copy_from_slice(tmp);
            // next
            continue;
        }
